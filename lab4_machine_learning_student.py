...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avg_rating = reduce(lambda x, y: x + y, ratings) / float(ratings_count)
    return (movie_id, (ratings_count, avg_rating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (movie_id, (movie_name, (ratings_count, avg))): (avg, movie_name, ratings_count)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg, movie_name, ratings_count): ratings_count > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((uid, mid), (p_rating, a_rating)): (p_rating - a_rating)**2))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.reduce(lambda x, y: x + y)

    # Count the number of entries for which you computed the total squared error
    numRatings = squaredErrorsRDD.map(lambda r: 1).reduce(lambda x, y: x + y)
...
validationForPredictRDD = validationRDD.map(lambda (user_id, movie_id, rating_id): (user_id, movie_id))
...
myModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda (user_id, movie_id, rating_id): (user_id, movie_id))
...
trainingAvgRating = (trainingRDD
                     .map(lambda (user_id, movie_id, rating): rating)
                     .reduce(lambda x, y: x + y)) / float(trainingRDD.count())
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (user_id, movie_id, rating): (user_id, movie_id, trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (movie_id, name): movie_id not in [mid for (uid, mid, rating) in myRatedMovies])
                      .map(lambda (movie_id, name): (myUserID, movie_id)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (mid, (ratings_count, avg_rating)): (mid, ratings_count))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (user_id, movie_id, rating): (movie_id, rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (movie_id, ((rating, ratings_count), movie_name)): (rating, movie_name, ratings_count))
                       .filter(lambda (rating, movie_name, ratings_count): ratings_count > 75)
                      )
